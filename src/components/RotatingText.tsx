...
const RotatingText = forwardRef<RotatingTextRef, RotatingTextProps>(
  (
    ...
    // Measure all texts width on mount
    useEffect(() => {
      const measuredWidths = measureRefs.current
        .filter(Boolean)
    ...
    const currentWidth = widths[currentTextIndex];
    ...
    return (
      ...
      >
        ...
        {/* Hidden spans to measure all text widths */}
        <div className="absolute opacity-0 pointer-events-none whitespace-nowrap" aria-hidden="true">
          {texts.map((text, index) => (
            <span 
              ...
              className={cn('inline-block whitespace-nowrap', mainClassName)}
            >
              {text}
            </span>
          ))}
        ...
        <AnimatePresence mode={animatePresenceMode} initial={animatePresenceInitial}>
          <motion.span
            ...
            className={cn(splitBy === 'lines' ? 'flex flex-col w-full' : 'flex whitespace-nowrap relative')}
            aria-hidden="true"
          >
